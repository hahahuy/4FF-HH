...
beforeAll(async () => {
    ...
    return {
      ...
      resolveDelete(raw, ctx) {
        ...
        if (a === "y" || a === "yes") {
          ...
          ctx.appendLine("Deleted (stub).", ["success"]);
        } else if (a === "n" || a === "no") {
          _pendingDelete = false;
  ...
  global.App.MessagePanel = (() => {
    ...
    return {
      confirmSend(content, ctx) {
        ...
        const MAX_SENDS = 5,
          WINDOW_MS = 40_000;
        const times = JSON.parse(localStorage.getItem(rateKey) || "[]").filter(
      ...
      resolvePendingCaptcha(raw, ctx) {
        const ans = Number.parseInt(raw.trim(), 10);
        if (Number.isNaN(ans) || ans !== _captchaAnswer) {
      ...
      getLastName() {
        try {
          return localStorage.getItem("mp_last_name") || null;
        } catch (e) {
          return null;
        }
      },
  ...
  global.App.EventBus.on("rawInput", ({ raw, ctx }) => {
    if (global.App.MessagePanel.hasPendingCaptcha()) {
      global.App.MessagePanel.resolvePendingCaptcha(raw, ctx);
      return true;
    }
    if (global.App.MessagePanel.hasPendingConfirm()) {
      global.App.MessagePanel.resolvePendingConfirm(raw, ctx);
      return true;
    }
    return false;
  });
