/* ============================================================
   ticker.js — Scrolling top-of-page news ticker
   Always visible on desktop (≥601px). User can dismiss for the
   session via the × button (persisted in localStorage).

   Segments (left → right):
     ₿  BTC           price + 24 h %   CoinGecko     every 15 min
     🥇 GOLD          PAXG + 24 h %    CoinGecko     every 15 min
     🤖 GH COMMITS    total commits     GitHub API    every 6 h
     💬 FORTUNE        rotates           local array   every 90 s
     📰 HN             top-5 headline    HN Firebase   once, rotates 60 s
     🔢 NUMBERS        date trivia       numbersapi    once on mount
     🌡 HCM weather    condition + temp  wttr.in       every 30 min
     · · ·
...
const Ticker = (() => {
  ...
  const FORTUNES = [
    ...
    { q: 'Sometimes it pays to stay in bed on Monday rather than spending the rest of the week debugging Sunday\'s code.', a: 'Dan Salomon' },
    ...
    { q: 'Always code as if the person who ends up maintaining your code is a violent psychopath who knows where you live.', a: 'John Woods' },
  ...
  const HINTS = [
    ...
    { html: '💡 <span class="tk-cmd">message --name &lt;you&gt;</span> — live chat', cmd: 'message --name ' },
  ...
  function _makeCopy(isClone) {
    ...
    copy.appendChild(_makeSegment('₿',  'BTC',    'tk-btc'));     copy.appendChild(_makeSep());
    copy.appendChild(_makeSegment('🥇', 'GOLD',   'tk-gold'));    copy.appendChild(_makeSep());
    copy.appendChild(_makeSegment('🤖', 'COMMITS','tk-ghcommits'));copy.appendChild(_makeSep());
    ...
    fSeg.innerHTML =
      `<span class="tk-lbl">💬</span> ` +
      `<span class="tk-val tk-fortune tk-loading">…</span>`;
    ...
    hnSeg.querySelector('.tk-hn').addEventListener('click', e => {
      if (!_hnHeadlines.length) { e.preventDefault(); return; }
      // href is already set by _updateHn — just let it open
    });
    copy.appendChild(hnSeg);
    copy.appendChild(_makeSep());

    copy.appendChild(_makeSegment('🔢', 'TRIVIA', 'tk-numbers'));  copy.appendChild(_makeSep());
    copy.appendChild(_makeSegment('🌡', 'HCM',    'tk-weather'));  copy.appendChild(_makeSep());
  ...
  function build() {
    ...
    if (!CSS.supports('selector(body:has(#ticker-bar))')) {
      document.body.style.paddingTop =
        (parseInt(document.body.style.paddingTop || '0', 10) + 28) + 'px';
    }
  }
  ...
  async function fetchHn() {
    try {
      ...
      _hnHeadlines = items
        ...
        .map(it => ({ title: it.title, url: it.url || `https://news.ycombinator.com/item?id=${it.id}` }));
