/* ============================================================
   js/commands/owner-commands.js — Owner-only commands
   Commands: auth, note, mv
   Hidden from `help` — require authentication.
   ============================================================ */
...
const OwnerCommands = {
  ...
  mv: {
    ...
    exec(args, path, ctx, { line, text, esc }) {
      ...
      const dstClean  = dst.replace(/^~\//, '');
      const dstParts  = dstClean.split('/').filter(Boolean);
      const dstDir    = dstParts.length > 1
        ? dstParts[0]
        : (dstParts[0] === filename ? '' : dstParts[0]);
      const dstFile   = dstParts[dstParts.length - 1];
